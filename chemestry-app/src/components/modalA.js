  ...
  const points=()=>{
    ...
    for(let x=0;x<arreglo.length-3;x++) {
      for(let y=1;y<arreglo.length-2;y++) {
        doIntersect(arreglo[y].x2,arreglo[y].x,arreglo[y+1].x2,arreglo[y+1].x,arreglo[x].y,arreglo[x].x,arreglo[x+1].y,arreglo[x+1].x);
        if(arreglo[y].x2.toFixed(2)==arreglo[x].y.toFixed(2)&&arreglo[y].x.toFixed(2)==arreglo[x].x.toFixed(2)){
            console.log("iguales");
